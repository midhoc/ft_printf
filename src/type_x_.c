...
/*   Updated: 2019/10/03 02:31:44 by hmidoun          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	put_sign(t_ft_printf *tst, unsigned long long int nbr,
						int flag, int flag_up)
{
...
static void	put_x_op_mns(t_ft_printf *tst, char *str,
							int flag_up, unsigned long long int nbr)
{
	int	c;
...
static void	put_x_op_0(t_ft_printf *tst, char *str,
							int flag_up, unsigned long long int nbr)
{
	int	c;
	...
	if (tst->precision >= 0)
	{
		tst->width -= (put_sign(tst, nbr, !tst->op_0, flag_up)
...
static void	put_x_op_(t_ft_printf *tst, char *str, int flag_up,
...
void		put_x(t_ft_printf *tst, unsigned long long int nbr, int flag_up)
{
	...
	if (!(str = ft_itoa_base(nbr, 16, flag_up)))
		return ;
	if (tst->op_mns)
		put_x_op_mns(tst, str, flag_up, nbr);
	else if (tst->op_0)
		put_x_op_0(tst, str, flag_up, nbr);
	else
		put_x_op_(tst, str, flag_up, nbr);
	free(str);
}
